...
function WordChanger({ isPlaying, setIsPlaying, wordSpeed, handleWordChange, contentArray, currentWordIndex, setCurrentWordIndex, editorFocus }) {
  const delay = 60000 / wordSpeed;
  const intervalRef = useRef(null);

  // Notify parent of word changes
  useEffect(() => {
    if (handleWordChange && contentArray.length > 0) {
      handleWordChange(currentWordIndex, contentArray.length); // Pass current index and total words
    }
  }, [currentWordIndex, handleWordChange, contentArray]);
